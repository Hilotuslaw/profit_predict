# -*- coding: UTF-8 -*-
# @FileName  :  views_re_predict.py
# @Time      :  2024/03/31 17:19:53
# @Author    :  Lotuslaw
# @Desc      :  predict again.
...
import json
import datetime
from models.req_resp_model import RePredictRequest, PredictResponse, SubPredictResponse
from utils.cal_util import cal_time_interval, fit_re_rate, revise_re_rate
from http import HTTPStatus


class RePredictView(views.MethodView):
    @validate()
    def post(self, body: RePredictRequest) -> PredictResponse:
        # TODO 参数解析
        ...
        # TODO 预测未来买量的留存
        dau_list = []
        for i, install in enumerate(new_install):
        ...
        official_income_after_share = np.round(dau_res * np.array(arpu_in) * (1 - local_pay_ratio) * (1 - official_pay_share_ratio), 0).astype(np.float32)
        local_income_after_share = np.round(dau_res * np.array(arpu_in) * local_pay_ratio * (1 - local_pay_share_ratio), 0).astype(np.float32)
        df_profit_day = pd.DataFrame(
            data=[
                np.array(days_predict),  # 日期
                np.arange(1, len(days_predict) + 1),  # 第N天
                np.array(new_install),  # 日新增
                dau_res,  # DAU
                np.array(arpu_in),  # 内购arpu
                np.array(arpu_ad),  # 广告arpu
                ...
                np.round(dau_res * np.array(arpu_ad), 0),  # 广告收入
                np.round(dau_res * np.array(arpu_ad) + dau_res * np.array(arpu_in), 0),  # 总流水
                np.round((dau_res * np.array(arpu_ad) + dau_res * np.array(arpu_in)) * cp_share_ratio, 0),  # CP分成流水
                np.round(official_income_after_share + local_income_after_share + dau_res * np.array(arpu_ad), 0),  # 分成后总收入
                np.array(cpi),  # CPI
                np.round(np.array(cpi) * np.array(new_install), 0),  # 投放支出
                np.round(np.array(cost_online_earning), 0),  # 网赚支出
                np.round(np.array(cost_market), 0),  # 市场支出
                np.round(official_income_after_share + local_income_after_share + dau_res * np.array(arpu_ad) - np.array(cpi) * np.array(new_install) - \
                         np.array(cost_online_earning) - np.array(cost_market) - (dau_res * (np.array(arpu_in) + np.array(arpu_ad))) * cp_share_ratio, 0),  # 每日盈亏
                np.round((official_income_after_share + local_income_after_share + dau_res * np.array(arpu_ad) - np.array(cpi) * np.array(new_install) - \
                          np.array(cost_online_earning) - np.array(cost_market) - (dau_res * (np.array(np.array(arpu_in) + np.array(arpu_ad)))) * cp_share_ratio).cumsum(), 0)  # 累计盈亏
            ]
        )
